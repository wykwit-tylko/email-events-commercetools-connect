  ...
  app.post("/event-proxy", async (request: Request, response: Response) => {
    ...
    try {
      ...
      const commerceNotification = extractCommerceNotificationBody({
        ...
        connectSubscriptionDestination: options.config.connectSubscriptionDestination,
      ...
      if (error instanceof PayloadTooLargeError || error instanceof DecodedPayloadTooLargeError) {
