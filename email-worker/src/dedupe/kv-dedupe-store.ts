...
export async function wasAlreadySent(env: Env, notificationId: string): Promise<boolean> {
...
export async function markSent(env: Env, notificationId: string): Promise<void> {
  const key = dedupeKey(notificationId);
  const ttl = dedupeTtlSeconds(env);
  await env.EMAIL_DEDUPE.put(key, new Date().toISOString(), { expirationTtl: ttl });
